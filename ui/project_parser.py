    ...
    if component.startswith("C_"):
        ...
        for part in project[component]:
            if part.startswith("P_"):
                ...
                # link to start working on the part
                link = part[2:].lower().replace(" ", "-")
                # url encode
                arguments = {
                    # ful lpath
                    'project_path': f"tmp/{project_file.name}",
                    'project': project["Name"],
                    'component': component[2:],
                    'part': part[2:]
                }
                import urllib.parse
                url = f"/part_editor?{urllib.parse.urlencode(arguments)}"
                st.markdown(f"[Start Working on this part]({url})")
